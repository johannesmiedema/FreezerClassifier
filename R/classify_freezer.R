...
classify_freezer <- function(data_MR1, data_MR2, sex, MR, shifter=FALSE, model = NULL, data = NULL){
  ...
  #Use standard anxiotraitR models if no retrained model is provided
  if (is.null(model)){
    ...
    if (MR == 1){
      ...
      if (sex == "female" & MR == 1){
        ...
        phenotypes <- stats::predict(anxiotraitR:::female_MR1, newdata = params_MR1)
      ...
      if (sex == "male" & MR == 1){
        ...
        phenotypes <- stats::predict(anxiotraitR:::male_MR1, newdata = params_MR1)
      }
    ...
    if (MR == 2 & shifter == T){
      ...
      if (sex == "female"){
        ...
        phenotypes <- stats::predict(anxiotraitR:::female_MR2, newdata = params_merged)
      }
      #Predict MR2 Phenotypes of males
      if (sex == "male"){
        message("Predicting male MR2 Phenotypes including shifters using MR1 and MR2 freezing data")
        phenotypes <- stats::predict(anxiotraitR:::male_MR2, newdata = params_merged)
      }
    ...
    if (MR == 2 & shifter == F){
      ...
      if (sex == "female"){
        ...
        phenotypes <- stats::predict(anxiotraitR:::female_MR2_noshifter, newdata = params_MR2)
        #Transform values as the model is a glm model
        phenotypes <- ifelse(phenotypes >= 0.5, "sustained", "phasic")
        phenotypes <- factor(phenotypes, levels = c("sustained", "phasic"))
      }
      ...
      if (sex == "male"){
        ...
        phenotypes <- stats::predict(anxiotraitR:::male_MR2_noshifter, newdata = params_MR2)
      }
      if (length(phenotypes) == nrow(data_MR2)){
        message("MR2 classification successfull")
        message("Classified  ", length(phenotypes), " animals. ")
  ...
  #Case where a individual model saved as RDS is provided:
    ...
    if (file.exists(model) && !dir.exists(model)){
      ...
      if(length(class(retrained.model)) == 1){phenotypes <- phenotypes$class}
